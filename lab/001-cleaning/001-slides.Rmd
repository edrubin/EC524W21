...
```{r, echo = F}
# Check results
starwars %>% select(name, mass, weight) %>% datatable(rownames = F, options = list(dom = 't'))
```
...
# Check if it worked!
starwars %>% select(name, contains("_color")) %>% datatable(rownames = F, options = list(dom = 't'))
```
...
# Check if it worked!
tmp %>% select(name, contains("_color")) %>% datatable(rownames = F, options = list(dom = 't'))
```
...
`mutate`.neg-left.b.pink[`_if`] is very similar to `mutate`.neg-left.b.orange[`_if`]. 
