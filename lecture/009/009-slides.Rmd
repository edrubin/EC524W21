...
p_load(
  ...
  lubridate, knitr,
  e1071, kernlab, caret, tidymodels,
...
```{r, hp-1a, echo = F, fig.height = 2.5}
ggplot(
  data = tibble(x = 1:10, y = 1),
  aes(x, y)
) +
annotate(
  geom = "segment",
...
```{r, hp-2a, echo = F, fig.height = 2.5}
ggplot(
  data = tibble(
    x = c(-2,-2, 6, 6,-2),
    y = c( 2, 0, 0, 2, 2),
    id = rep(as.character(1), each = 5),
  ),
  aes(x, y, fill = id)
) +
geom_polygon(
...
```{r, hp-2b, echo = F, fig.height = 2.5}
ggplot(
  data = tibble(
    x = c(0, 4,-2,-2, 0,  0, 4, 6, 6, 0),
    y = c(0, 2, 2, 0, 0,  0, 2, 2, 0, 0),
    id = rep(as.character(1:2), each = 5),
  ),
  aes(x, y, fill = id, alpha = id)
) +
geom_polygon(
  aes(group = id),
  color = NA
...
scale_fill_manual(values = c(orange, slate)) +
...
# Save
htmlwidgets::saveWidget(p, "fig-sep-3d.html")
```
...
- .b.orange[Polynomial] <br> .orange[`svm_linear(cost, degree, scale_factor)`] with `"kernlab"` engine
...
```{r, plot-svm-linear, echo = F}
ggplot(
  data = tuning_linear_svm %>% collect_metrics() %>% clean_names(),
  aes(x = cost, y = mean, color = metric)
) +
geom_line(size = 0.5) +
geom_point(size = 4.5) +
scale_x_log10("Cost", labels = scales::comma) +
scale_y_continuous("Accuracy") +
theme_minimal(base_family = "Fira Sans Book") +
scale_color_manual(
  "Metric",
  values = c(slate, purple),
  labels = c("Accuracy", "F Measure")
) +
...
```{r, plot-rbf-poly, echo = F}
ggplot(
  data = tuning_poly_svm %>% collect_metrics() %>% clean_names() %>% dplyr::filter(metric == "accuracy"),
  aes(x = cost, y = mean, color = as.character(degree))
) +
geom_line(size = 0.5) +
geom_point(size = 4.5) +
scale_x_log10("Cost", labels = scales::comma) +
...
scale_color_manual(
  "Degree",
  values = c(slate, orange, red_pink)
) +
theme(legend.position = "bottom")
```
...
```{r, plot-svm-rbf, echo = F}
ggplot(
  data = tuning_rbf_svm %>% collect_metrics() %>% clean_names() %>% dplyr::filter(metric == "accuracy"),
  aes(x = cost, y = mean, color = as.character(rbf_sigma))
) +
geom_line(size = 0.5) +
geom_point(size = 4.5) +
scale_x_log10("Cost", labels = scales::comma) +
...
scale_color_manual(
  "RBF Sigma",
  values = c(slate, orange, red_pink, purple)
) +
theme(legend.position = "bottom")
```
