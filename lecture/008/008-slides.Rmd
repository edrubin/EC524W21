...
# Define CV splits
heart_cv = heart_df %>% vfold_cv(v = 5)
# Define workflow
heart_bag_wf = workflow() %>% 
  add_model(heart_bag) %>% 
  add_recipe(heart_recipe)
# Fit/assess with CV
test = heart_bag_wf %>% fit_resamples(heart_cv)
test = heart_bag_wf %>% fit(heart_df)
```
...
Unfortunately, this combination of `rpart`/`baguette`/`parsnip`/`yardstick` doesn't ([currently](https://github.com/tidymodels/baguette/issues/33)) offer OOB-based metrics. 😞
...
But before .it[that], let's talk about variable importance.
